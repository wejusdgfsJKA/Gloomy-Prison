...
public class AwarenessSystem : MonoBehaviour
{
            ...
            for (int i = 0; i < DetectionManager.instance.teams.Length; i++)
            {
                    ...
                    gameObject.layer.ToString())
                {
                    ...
                    foreach (Transform entity in team)
                    {
                        ...
                        {
                            //we can see this entity or it is near us
                            HasDetected(entity);
                        }
                    }
                }
    ...
    protected void ProcessInformation()
    {
        ...
        foreach (var data in targets)
        {
            ...
            {
                //we will forget this target, but we can't remove it while in the
                //foreach loop
                queue.Enqueue(data.Key);
                continue;
            }
            ...
            if (ClosestTarget == null || !targets.ContainsKey(ClosestTarget))
            {
                ClosestTarget = data.Key;
            }
            else
            {
                float dist = (transform.position - targets[ClosestTarget].KnownPos).sqrMagnitude;
